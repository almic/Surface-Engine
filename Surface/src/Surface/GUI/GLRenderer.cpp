...
//  2019-05-04: OpenGL: Removed support for versions below OpenGL 4.1 (410 core), retained support for ES 3.0 however. Default version is now 430
...
bool    Gl_Gui_Init(const char* glsl_version)
{
	...
	strcpy_s(g_GlslVersionString, 32, glsl_version);
	strcat_s(g_GlslVersionString, 32, "\n");
...
static bool CheckShader(GLuint handle, const char* desc)
{
	...
	if ((GLboolean)status == GL_FALSE)
		fprintf(stderr, "ERROR: Gui_CreateDeviceObjects: failed to compile %s!\n", desc);
	if (log_length > 0)
	{
		ImVector<char> buf;
...
static bool CheckProgram(GLuint handle, const char* desc)
{
	...
	if ((GLboolean)status == GL_FALSE)
		fprintf(stderr, "ERROR: Gui_CreateDeviceObjects: failed to link %s! (with GLSL '%s')\n", desc, g_GlslVersionString);
	if (log_length > 0)
	{
		ImVector<char> buf;
		buf.resize((int)(log_length + 1));
